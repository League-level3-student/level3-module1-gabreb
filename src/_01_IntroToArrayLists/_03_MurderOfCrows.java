...
public class _03_MurderOfCrows {
   ...
   for (int i = 0; i < theMurder.size(); i++) {
	   ArrayList<String> anewone = theMurder.get(i).getStomachContents();
	   for (int z = 0; z < anewone.size(); z++) {
		   if (anewone.get(z).equalsIgnoreCase("diamond")) {
			   JOptionPane.showMessageDialog(null, "Crow "+ (z+1)+ " stole and ate the diamond. " + (z) + " innocent birds died for this treasure!");
			   break;
		   }
	   }
   }
