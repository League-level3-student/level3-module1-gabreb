...
public class DynamicIntegerArray {
	...
	public void add(int v) {
		...
		int[] arrayonelonger = new int[array.length + 1];
		// B. set the last element of your new array to
		// the value passed into the method
		arrayonelonger[arrayonelonger.length - 1] = v;
		// C. iterate through the member array and
		// copy every element from the member array
		// to the new array
		for (int i = 0; i < array.length; i++) {
			arrayonelonger[i] = array[i];
		}
		// D. set the member array equal to the new array.
		array = arrayonelonger;
	}
	...
	public void insert(int v, int location) {
		// A. create and initialize a new int array to be one
		// element longer than the member array
		int[] arrayforanother = new int[array.length + 1];
		// B. Make a for loop that iterates through the new array
		for (int i = 0; i < arrayforanother.length; i++) {
				if (i < location) {
					arrayforanother[i] = array[i];
				}
				else if (i > location) {
					arrayforanother[i] = array[i-1];
				}
				else {
					arrayforanother[i] = v;
				}
		}
				array = arrayforanother;
...
int[] arrayminus1 = new int[array.length-1];
		// B. make a for loop to iterate through the member array
for (int i = 0; i < array.length; i++) {
	if (i < location) {
		arrayminus1[i] = array[i];
	}
	else if (i > location) {
		arrayminus1[i-1] = array[i];
	}
	else {
		continue;
	}
